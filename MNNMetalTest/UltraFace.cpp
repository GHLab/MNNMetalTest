...
int UltraFace::detect(const uint8_t *data, const int width, const int height, const int stride, std::vector<FaceInfo> &face_list) {
	image_h = height;
	image_w = width;
	...
	std::shared_ptr<MNN::CV::ImageProcess> pretreat(
			MNN::CV::ImageProcess::create(MNN::CV::BGRA, MNN::CV::RGB, mean_vals, 3,
										  norm_vals, 3));
	
	pretreat->setMatrix(trans);
	pretreat->convert(data, width, height, stride, input_tensor);
